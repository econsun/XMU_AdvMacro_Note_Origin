...
\[
\max_{\left\{C_t,N_t,B_t\right\}}
E_0\sum_{t=0}^{\infty}\beta^t U\left(C_t,N_t\right)
\]
...
\left[
U\left(C_t,N_t\right)+\lambda_t\left(B_{t-1}+W_tN_t-T_t-P_tC_t-Q_tB_t\right)
\right].
\]
...
\quad\Rightarrow\quad
-\beta^t\lambda_t Q_t+\beta^{t+1}E_t\left[\lambda_{t+1}\right]=0
\quad\Rightarrow\quad
...
\quad\Rightarrow\quad
\frac{\partial \log N_t}{\partial \log \left(W_t/P_t\right)}
=
\frac{1}{\varphi}
\]
...
FOC:
\[
\frac{W_t}{P_t}=\left(1-\alpha\right)A_tN_t^{-\alpha}
\qquad
...
\]
\[
\text{Labor market clearing: }\qquad N_t^{\left(d\right)} = N_t^{\left(s\right)}
\left(\text{implicitly holds}\right)
\]
\[
...
\frac{W_t}{P_t} &= \left(1-\alpha\right)A_t N_t^{-\alpha},
...
\log A_t &= \left(1-\rho_a\right)\log \bar{A} + \rho_a \log A_{t-1} + \varepsilon_t,
...
\left\{
\begin{aligned}
W_t^r &= C_t^{\sigma} N_t^{\varphi} \qquad \left(1\right)\\[4pt]
Y_t &= A_t N_t^{1-\alpha} \qquad \left(2\right)\\[4pt]
W_t^r &= \left(1-\alpha\right)A_t N_t^{-\alpha} \qquad \left(3\right)\\[4pt]
Y_t &= C_t \qquad \left(4\right)
\end{aligned}
\right.
\]
...
From (2), (3), and \(Y=C\):
\[
\left(1-\alpha\right)\bar N^{-\alpha}
=
...
=
\bar N^{\sigma\left(1-\alpha\right)+\varphi}.
\]
...
1-\alpha
=
\bar N^{\sigma\left(1-\alpha\right)+\varphi+\alpha},
\]
so
\[
\bar N
=
\left(1-\alpha\right)^{\frac{1}{\sigma\left(1-\alpha\right)+\varphi+\alpha}}.
\]
...
\bar Y=\bar N^{1-\alpha}
=
\left(1-\alpha\right)^{\frac{1-\alpha}{\sigma\left(1-\alpha\right)+\varphi+\alpha}},
\qquad
\bar C=\bar Y.
\]
and
...
Then
\[
\sigma \hat{a}_t + \sigma\left(1-\alpha\right)\hat{n}_t + \varphi \hat{n}_t
...
\hat{c}_t=\hat{y}_t
=
\frac{1+\varphi}{\sigma\left(1-\alpha\right)+\varphi+\alpha}\,\hat{a}_t
\qquad
\text{coefficient >0}
\]
...
\hat{w}_t^{\,r}
=
\frac{\varphi+\sigma}{\sigma\left(1-\alpha\right)+\varphi+\alpha}\,\hat{a}_t
\qquad
\text{coefficient >0}
\]
\[
\hat{n}_t
=
\frac{1-\sigma}{\sigma\left(1-\alpha\right)+\varphi+\alpha}\,\hat{a}_t
\qquad
...
\left(\frac{C_{t+1}}{C_t}\right)^{-\sigma}
&=
e^{-\sigma \log C_{t+1}} / e^{-\sigma \log C_t}
=
e^{-\sigma\left(\log C_{t+1}-\log C_t\right)},
\\[6pt]
\frac{1}{\Pi_{t+1}}
&=
\frac{P_t}{P_{t+1}}
...
=
e^{-\rho} E_t \!\left[
e^{-\sigma\left(\hat c_{t+1}-\hat c_t\right)-\pi_{t+1}}
\right]
}
\tag{**}
\]
...
\[
-i_t = -\rho + E_t\!\left[-\sigma\left(\hat c_{t+1}-\hat c_t\right)-\pi_{t+1}\right]
\tag{1}
\]
...
+
E_t\!\left[
-\sigma\left(\log \bar{C}-\log \bar{C}\right)-\bar{\pi}
\right],
\\[6pt]
\bar{\pi}
&=
...
\[
\begin{aligned}
\left(1\right)+\left(2\right)\quad \Rightarrow\quad
-\hat{i}_t+\bar{i}
&=
E_t\!\left[-\sigma\left(\log C_{t+1}-\log C_t\right)-\pi_{t+1}\right].
\end{aligned}
\]
...
-\hat{i}_t
&=
E_t\!\left[-\sigma\left(\hat{c}_{t+1}-\hat{c}_t\right)-\pi_{t+1}\right],
\\[6pt]
\Rightarrow\quad
-\hat{i}_t
&=
-\sigma E_t\Delta \hat{c}_{t+1}-E_t\left[\pi_{t+1}\right],
\\[6pt]
\Rightarrow\quad
\sigma E_t\Delta \hat{c}_{t+1}
&=
\hat{i}_t-E_t\left[\pi_{t+1}\right],
\\[6pt]
\hat{\pi}_{t+1}
&=
\pi_{t+1}-\bar{\pi}
...
\\[6pt]
\Rightarrow\quad
\hat{i}_t-E_t\left[\hat{\pi}_{t+1}\right]
&=
\sigma E_t\Delta \hat{c}_{t+1}.
\end{aligned}
\tag{***}
...
\[
\text{Since }
r_t=i_t-E_t\left[\pi_{t+1}\right],
\tag{3}
\]
\[
...
\]
\[
\left(3\right)-\left(4\right)\;\Rightarrow\;
\hat{r}_t=\hat{i}_t-E_t\left[\hat{\pi}_{t+1}\right]
\]
\[
\text{Plug in }\left(***\right)\;\Rightarrow\;
...
\[
\Rightarrow\;
\hat{r}_t=\sigma \psi_{ya}\, E_t\left[\Delta \hat{a}_{t+1}\right]
\]
\[
\text{If the shock is transitory / mean reverting}
\quad\Rightarrow\quad
E_t\left[\Delta \hat{a}_{t+1}\right]<0
\quad\Rightarrow\quad
r_t<0.
\]
...
\[
\max_{\left\{C_t,M_t,N_t\right\}_{t=0}^{\infty}}
E_0\sum_{t=0}^{\infty}\beta^t\,
...
B.C.:
\[
P_tC_t + Q_tA_{t+1} + \left(1-Q_t\right)M_t = A_t + W_tN_t + T_t
\]
Notes 1:
\[
...
+
\lambda_t\left(
A_t+W_tN_t+T_t-P_tC_t-Q_tA_{t+1}-\left(1-Q_t\right)M_t
\right)
\right].
\]
...
\quad\Rightarrow\quad
U_{\mathbb{M}_t}\frac{1}{P_t}-\lambda_t\left(1-Q_t\right)=0
\]
\[
\frac{\partial \mathcal L}{\partial C_t}=0
\quad\Rightarrow\quad
...
\section{Separable utility case (still dichotomy)}
...
\[
\begin{aligned}
\text{Let } f\left(i\right) &= \log\left(1-e^{-i}\right),\\[4pt]
f'\left(i\right) &= \frac{e^{-i}}{1-e^{-i}} = \frac{1}{e^i-1},\\[4pt]
f''\left(i\right) &= -\frac{e^i}{\left(e^i-1\right)^2},\\[6pt]
\log\left(1-e^{-i_t}\right)
&\approx
\log\left(1-e^{-\bar{i}}\right)
+\frac{1}{e^i-1}\left(i_t-\bar{i}\right).
\end{aligned}
\]
\[
\log\left(1-e^{-\bar{i}}\right) \text{ is constant, so it cancels out in derivation.}
\]
\mbox{}\\
If \(\sigma=\nu\), define
\[
...
\section{Inseparable utility case}
Utility:
\[
U\!\left(C_t,\mathbb{M}_t,N_t\right)
=
...
For \(\nu\neq 1\),
\[
X_t=\left[\left(1-\theta\right)C_t^{\,1-\nu}+\theta \mathbb{M}_t^{\,1-\nu}\right]^{\frac{1}{1-\nu}}.
\]

Then
\[
U_{C,t}=X_t^{-\sigma}\cdot \frac{1}{1-\nu}\cdot X_t^{\nu}\cdot \left(1-\theta\right)\left(1-\nu\right)C_t^{-\nu}
\]
so
\[
U_{C,t}=\left(1-\theta\right)X_t^{\,\nu-\sigma}C_t^{-\nu}.
\]
...
\varepsilon
=
\frac{d\ln\left(\mathbb{M}/C\right)}{d\ln MRS_{C,\mathbb{M}}}
=
\frac{1}{\nu}.
\]
...
\hat{w}_t-\hat{p}_t
=
\sigma\hat{c}_t+\varphi \hat{n}_t+\left(\nu-\sigma\right)\left(\hat{c}_t-\hat{x}_t\right)
\qquad \cdots\; \left(**\right)
\]
\[
X_t^{1-\nu}
=
\left(1-\theta\right)C_t^{1-\nu}
+
\theta \mathbb{M}^{1-\nu}
\]
\[
\textcolor{red}{\text{S.S.:}\quad
\left(1-\theta\right)\bar{C}^{\,1-\nu}
=
\bar{X}^{\,1-\nu}
-
\theta \bar{\mathbb{M}}^{\,1-\nu}}
...
\[
\Rightarrow
\bar X^{1-\nu}\left(\hat c_t-\hat x_t\right)
=
\theta\bar{\mathbb{M}}^{1-\nu}
\left[
\hat c_t-\left(\hat m_t-\hat p_t\right)
\right].
\]
...
\[
\text{Define}\qquad
\left(\frac{\bar{M}}{\bar{P}}\right)/\bar{C}
=
k_m
\]
...
=
\frac{
\theta\left(\left(\frac{\bar{M}}{\bar{P}}\right)/\bar{C}\right)^{1-\nu}
}{
1-\theta+\theta\left(\left(\frac{\bar{M}}{\bar{P}}\right)/\bar{C}\right)^{1-\nu}
}
\]
...
=
\left[
\frac{\theta}{\left(1-\beta\right)\left(1-\theta\right)}
\right]^{1/{\nu}}
\]
...
\[
\text{Plug } \left(***\right) \text{ into } \left(**\right), \text{ we have}
\]
...
+
\left(\nu-\sigma\right)\chi
\left[\hat{c}_t-\left(\hat{m}_t-\hat{p}_t\right)\right]
\]

\[
\text{Plug } \left(*\right) \text{ into the above equation:}
\]
...
+
\left(\nu-\sigma\right)\chi
\left[\hat{c}_t-\hat{y}_t+\eta \hat{i}_t\right]
\]
...
\[
\Rightarrow\quad
\chi=\frac{k_m\left(1-\beta\right)}{1+k_m\left(1-\beta\right)}
\]

\[
k_m\left(1-\beta\right)
=
\left(\frac{\theta}{1-\theta}\right)^{1/\nu}\left(1-\beta\right)^{1-\frac{1}{\nu}}
\]
...
\chi \eta \left(\nu-\sigma\right)
=
\frac{k_m\left(1-\beta\right)}{1+k_m\left(1-\beta\right)}
\cdot
\frac{1}{\nu\left(e^{\bar{i}}-1\right)}
\cdot
\left(\nu-\sigma\right)
...
=
\frac{k_m\beta\left(\nu-\sigma\right)}
{\nu\left[1+k_m\left(1-\beta\right)\right]}
\]
\[
\operatorname{sign}\left(\omega\right)=\operatorname{sign}\!\left(1-\frac{\sigma}{\nu}\right)
\qquad\Rightarrow\qquad
\omega>0
\iff
\nu>\sigma
...
\hat{c}_t-\hat{x}_t
=
\chi\left[\hat{c}_t-\left(\hat{m}_t-\hat{p}_t\right)\right]
\]
...
E_t\!\left[
e^{
-\nu\left(\log C_{t+1}-\log C_t\right)
+\left(\nu-\sigma\right)\left(\log X_{t+1}-\log X_t\right)
-\pi_{t+1}
}
\right]
\]
...
+
E_t\!\left[
-\nu\left(\log \bar{C}-\log \bar{C}\right)
+\left(\nu-\sigma\right)\left(\log \bar{X}-\log \bar{X}\right)
-\bar{\pi}
\right]
\end{aligned}
\right.
...
=
E_t\!\left[
\nu\left(\hat{c}_{t+1}-\hat{c}_t\right)
-\left(\nu-\sigma\right)\left(\hat{x}_{t+1}-\hat{x}_t\right)
+\pi_{t+1}
\right]
\]
...
=
\nu E_t\left(\hat{c}_{t+1}-\hat{c}_t\right)
-\left(\nu-\sigma\right)E_t\left(\hat{x}_{t+1}-\hat{x}_t\right)
\]
...
-\left(\nu-\sigma\right)E_t\Delta \hat{c}_{t+1}
+
\textcolor{red}{\left(\nu-\sigma\right)\chi\eta}\,E_t\Delta \hat{i}_{t+1}
\]
...
\[
\textcolor{red}{\text{In }\left(*\right)\text{ we apply market clearing }\hat{c}_t=\hat{y}_t}
\]
...
\qquad
\hat{y}_t=\hat{a}_t+\left(1-\alpha\right)\hat{n}_t
\]

\[
\Rightarrow\quad
\hat{n}_t=\frac{1}{1-\alpha}\left(\hat{y}_t-\hat{a}_t\right)
\]
...
\[
\Rightarrow\quad
\left(1-\sigma\right)\hat{y}_t-\left(1+\varphi\right)\hat{n}_t
=
\omega \hat{i}_t
\]
...
\hat{y}_t
=
\hat{a}_t+\left(1-\alpha\right)\hat{n}_t
\]
...
\hat{y}_t
=
\frac{1+\varphi}{\alpha\left(1-\sigma\right)+\sigma+\varphi}\hat{a}_t
-
\frac{\left(1-\alpha\right)\omega}{\alpha\left(1-\sigma\right)+\sigma+\varphi}\hat{i}_t
\qquad \left(1\right)
\]
\[
\Rightarrow\quad
...
\[
\Rightarrow\quad
\left(1-\sigma\right)\hat{y}_t-\left(1+\varphi\right)\hat{n}_t=\omega \hat{i}_t
\]
...
\qquad
\textcolor{red}{\left(\text{interest rate / monetary policy affects real output}\right)}
\]
...
\left(\hat{y}_t,\pi_t,\hat{i}_t\right).
\]
...
{\pi}_t
&=
-\frac{\sigma\left(1-\rho_a\right)\psi_{ya}}
{\phi_{\pi}\left(1+\omega\psi\right)\left(1-\theta\rho_a\right)}\,\hat{a}_t
-\frac{1+\left(1-\rho_v\right)\omega\psi}
{\phi_{\pi}\left(1+\omega\psi\right)\left(1-\theta\rho_v\right)}\,v_t,
\\[8pt]
\hat{i}_t
&=
-\frac{\sigma\left(1-\rho_a\right)\psi_{ya}}
{\left(1+\omega\psi\right)\left(1-\theta\rho_a\right)}\,\hat{a}_t
-\frac{\rho_v}
{\phi_n\left(1+\omega\psi\right)\left(1-\theta\rho_v\right)}\,v_t,
\\[8pt]
\hat{y}_t
&=
\psi_{ya}\left(
1+
\frac{\sigma\left(1-\rho_a\right)\psi_{yi}}
{\left(1+\omega\psi\right)\left(1-\theta\rho_a\right)}
\right)\hat{a}_t
+
\frac{\rho_v\psi_{yi}}
{\phi_{\pi}\left(1+\omega\psi\right)\left(1-\theta\rho_v\right)}\,v_t.
...
{\left(1+\omega\varphi\right)\phi_{\pi}},
\qquad
\psi
=
\frac{\alpha+\varphi}{\sigma\left(1-\alpha\right)+\alpha+\varphi}.
\]
...
\par\vspace{12pt plus 4pt minus 4pt}
...
=
1-
\frac{dE_t\left(\pi_{t+1}\right)/d\nu_t}{d\hat{i}_t/d\nu_t}
=
-\left(1-\rho_{\nu}\right)\omega\psi
<0.
\]
...
\[
\left({r}_t={i}_t-E_t\pi_{t+1}\right).
\]
...
\[
\hat{i}_t = E_t\left[\pi_{t+1}\right] + \hat{r}_t,
\qquad\Rightarrow\qquad
E_t\left[\pi_{t+1}\right] = \hat{i}_t - \hat{r}_t.
\]
...
\[
\Rightarrow\qquad
E_t\left[\pi_{t+1}\right]=-\hat{r}_t
\qquad\Rightarrow\qquad
\pi_{t+1}=-\hat{r}_t+\zeta_{t+1},
\qquad
E_t\left[\zeta_{t+1}\right]=0.
\]
...
we obtain
\[
\phi_{\pi}\pi_t = E_t\left[\pi_{t+1}\right] + \hat{r}_t.
\tag{*}
\]
...
Iterate forward:
\[
\pi_t=\frac{1}{\phi_{\pi}}E_t\left[\pi_{t+1}\right]+\frac{1}{\phi_{\pi}}\hat r_t
\quad\Rightarrow\quad
\pi_t=\sum_{k=0}^{\infty}\phi_{\pi}^{-\left(k+1\right)}E_t\left[\hat r_{t+k}\right],
\]
...
Bond Euler:
\[
\hat i_t-E_t\left[\pi_{t+1}\right]=\sigma E_t\Delta \hat c_{t+1},
\qquad
\hat i_t-E_t\left[\pi_{t+1}\right]=\hat r_t,
\]
so
\[
...
then
\[
E_t\left[\hat a_{t+1}\right]=\rho_a\hat a_t,
\qquad
E_t\left[\Delta \hat a_{t+1}\right]
=
E_t\left[\hat a_{t+1}-\hat a_t\right]
=
\left(\rho_a-1\right)\hat a_t.
\]

\noindent
Hence,
\[
\hat r_t=\sigma\psi_{ya}\left(\rho_a-1\right)\hat a_t.
\]
\smallskip
...
=K\rho_a\hat a_t-\sigma\psi_{ya}\left(1-\rho_a\right)\hat a_t,
\]
so
\[
\phi_{\pi}K-K\rho_a=-\sigma\psi_{ya}\left(1-\rho_a\right),
\]
and therefore
\[
K=-\frac{\sigma\psi_{ya}\left(1-\rho_a\right)}{\phi_{\pi}-\rho_a}.
\]
Thus,
\[
\pi_t=-\frac{\sigma\psi_{ya}\left(1-\rho_a\right)}{\phi_{\pi}-\rho_a}\hat a_t.
\]
...
\textbf{Case 2:}\quad $\phi_{\pi}\le 1$
...
\[
\phi_{\pi}\pi_t=E_t\left[\pi_{t+1}\right]+\hat r_t.
\]
The stable solution is
\[
\pi_{t+1}=\phi_{\pi}\pi_t-\hat r_t+\xi_{t+1},
\qquad
E_t\left[\xi_{t+1}\right]=0,
\]
...
+\lambda_t\left(A_tN_t^{1-\alpha}-C_t\right).
\]
...
\[
i_t=\phi\,\left(r_{t-1}+\pi_t\right),
\qquad \phi>1.
\]
Then
\[
...
\]
\[
E_t\left[i_{t+s}\right] = \phi^s i_t.
\]
...
-\rho
+
E_t\!\left[-\sigma\left(\log\bar C-\log\bar C\right)-\bar\pi\right],
\]
with
\[
\bar\pi=0.
...
\[
\sigma E_t\left(\hat c_{t+1}-\hat c_t\right)=\hat i_t-E_t\pi_{t+1}.
\]
Thus
\[
\hat c_t
...
E_t\hat c_{t+1}
-
\frac{1}{\sigma}\left[\hat i_t-E_t\pi_{t+1}\right].
\]
Since
\[
\hat y_t=\hat c_t,
...
\[
E_t\!\left[\frac{\left(1+i_t\right)/P_{t+1}}{1/P_t}\right]
=
...
Also,
\[
r_t-\bar r=i_t-\bar i-E_t\left[\pi_{t+1}-\bar\pi\right]
\;\Rightarrow\;
...
\[
Q_t=\beta E_t\!\left[\left(\frac{C_{t+1}}{C_t}\right)^{-\sigma}\frac{P_t}{P_{t+1}}\right]
=\beta E_t\!\left[e^{-\sigma\left(\log C_{t+1}-\log C_t\right)-\pi_{t+1}}\right],
\]
so
\[
e^{-i_t}=e^{-\rho}E_t\!\left[e^{-\sigma\left(\log C_{t+1}-\log C_t\right)-\pi_{t+1}}\right].
\]
...
Then
\[
e^{-i_t}=e^{-\rho}E_t\!\left[e^{-\sigma\left(\hat c_{t+1}-\hat c_t\right)-\pi_{t+1}}\right].
\]
...
At steady state,
\[
-\bar i=-\rho+E_t\left[0-0\right] \;\Rightarrow\; \bar i=\rho.
\]
Hence
\[
-\left(i_t-\bar i\right)=E_t\!\left[-\sigma\left(\hat c_{t+1}-\hat c_t\right)\right]-E_t\left[\pi_{t+1}-\bar\pi\right],
\]
or equivalently,
\[
\hat i_t=\sigma E_t\left[\Delta \hat c_{t+1}\right]+E_t\left[\hat\pi_{t+1}\right],
\qquad
\Delta \hat c_{t+1}=\hat c_{t+1}-\hat c_t.
\]
Therefore,
\[
\sigma E_t\left[\Delta \hat c_{t+1}\right]
=\hat i_t-E_t\left[\hat\pi_{t+1}\right]
=\hat r_t.
\]
Using \(\hat c_t=\hat y_t\), we obtain
\[
\sigma E_t\left[\Delta \hat y_{t+1}\right]=\hat r_t.
\]

\noindent
Since
\[
E_t\left[\Delta \hat y_{t+1}\right]=E_t\hat y_{t+1}-\hat y_t,
\qquad
\hat y_t=\psi_{ya}\hat a_t
\;\Rightarrow\;
E_t\left[\Delta \hat y_{t+1}\right]=\psi_{ya}E_t\left[\Delta \hat a_{t+1}\right],
\]
it follows that
\[
\hat r_t=\sigma\psi_{ya}E_t\left[\Delta \hat a_{t+1}\right].
\]

\noindent
Let
\[
\log A_{t+1}=\left(1-\psi\right)\log\bar A+\varphi\log A_t+\varepsilon_{t+1},
\qquad
\bar A=1.
\]
Then
...
Taking expectations,
\[
E_t\left[\hat a_{t+1}\right]
=\psi\hat a_t+E_t\left[\varepsilon_{t+1}\right],
\qquad
\varepsilon_t\sim \text{i.i.d. }N\left(0,\sigma^2\right)
\;\Rightarrow\;
E_t\left[\varepsilon_{t+1}\right]=0,
\]
so
\[
E_t\left[\hat a_{t+1}\right]=\psi\hat a_t,
\qquad
E_t\left[\hat a_{t+1}-\hat a_t\right]=\left(\psi-1\right)\hat a_t.
\]
Let
\[
\Delta \hat a_{t+1}=\hat a_{t+1}-\hat a_t
\;\Rightarrow\;
E_t\left[\Delta \hat a_{t+1}\right]=\left(\psi-1\right)\hat a_t.
\]

\noindent
Plugging this into \(\hat r_t=\sigma\psi_{ya}E_t\left[\Delta \hat a_{t+1}\right]\), we get
\[
\hat r_t
=\sigma\psi_{ya}E_t\left[\Delta \hat a_{t+1}\right]
=\sigma\psi_{ya}\left(\psi-1\right)\hat a_t.
\]
Since \(\psi\in\left(0,1\right)\) and \(\psi_{ya}>0\), the coefficient on \(\hat a_t\) is negative.
