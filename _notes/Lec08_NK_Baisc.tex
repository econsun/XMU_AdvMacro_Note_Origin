...
\begin{enumerate}[leftmargin=1.7em,itemsep=0.35em,topsep=0.2em]
    \item Monopolistic competition: $P^m>P^c, Y^m<Y^c$, \red{monopolistic competition}
    \hfill \red{inefficient response to shocks.}

    \item Nominal rigidities: staggered price / wage, short-run price rigidity.

    \item Short-run non-neutrality of monetary policy
...
\end{enumerate}
\[
\hat i_t=f\left(\Omega_t\right)+v_t,\qquad \red{\Omega_t:\text{ information set / state vector.}}
\tag{3}
\]
Four markets: goods, labor, bonds, money (cashless).\\
\par\smallskip
Differentiated goods: continuum of varieties $i\in\left[0,1\right]$.\\
\par\smallskip
...
\par\smallskip
Nominal prices are sticky: a fraction $\left(1-\theta\right)$ of firms can reset their prices each period (Calvo pricing).\\
\par\smallskip
A continuum of infinitely-lived households.\\
\par\smallskip
A continuum of firms represented by $\left[0,1\right]$ goods $\Rightarrow$ \red{1 firm produces 1 differentiated good and sets price.}

\section{Household's problem}
\[
\max_{C_t,N_t,B_t}\;\E_t\sum_{t=0}^{\infty}\beta^tU\left(C_t,N_t\right),
\]
\[
\text{s.t.}\quad \boxit{P_t}C_t+Q_tB_t=B_{t-1}+W_tN_t+T_t.
\]
\blue{$P_t$: aggregate price index of the consumption bundle, $P_t C_t=\int_0^1P_t\left(i\right)C_t\left(i\right)\dd i$.}
\[
C_t=\left(\int_0^1 C_t\left(i\right)^{\frac{\varepsilon-1}{\varepsilon}}\dd i\right)^{\frac{\varepsilon}{\varepsilon-1}},
\]
...
\[
E=\min_{C_t\left(i\right)}\int_0^1P_t\left(i\right)C_t\left(i\right)\dd i=\text{total expenditure}
\quad \text{s.t.}\quad
C_t=\left(\int_0^1 C_t\left(i\right)^{\frac{\varepsilon-1}{\varepsilon}}\dd i\right)^{\frac{\varepsilon}{\varepsilon-1}}.
\]
\[
\mathcal L=\int_0^1P_t\left(i\right)C_t\left(i\right)\dd i-P_t\left[\left(\int_0^1 C_t\left(i\right)^{\frac{\varepsilon-1}{\varepsilon}}\dd i\right)^{\frac{\varepsilon}{\varepsilon-1}}-C_t\right].
\]
\begin{tightalign}
...
\\[0.6em]
&\Rightarrow
P_t\left(i\right)-P_tC_t^{1/\varepsilon}C_t\left(i\right)^{-1/\varepsilon}=0
\\[0.6em]
&\Rightarrow
C_t\left(i\right)=
\left(\frac{P_t\left(i\right)}{P_t}\right)^{-\varepsilon}C_t.
\end{tightalign}
\begin{tightalign}
C_t\left(i\right)^{\frac{\varepsilon-1}{\varepsilon}}
&=\left(\frac{P_t\left(i\right)}{P_t}\right)^{1-\varepsilon}C_t^{\frac{\varepsilon-1}{\varepsilon}},\\
\int_0^1C_t\left(i\right)^{\frac{\varepsilon-1}{\varepsilon}}\dd i
&=C_t^{\frac{\varepsilon-1}{\varepsilon}}\int_0^1\left(\frac{P_t\left(i\right)}{P_t}\right)^{1-\varepsilon}\dd i,\\
C_t^{\frac{\varepsilon-1}{\varepsilon}}
&=C_t^{\frac{\varepsilon-1}{\varepsilon}}\int_0^1\left(\frac{P_t\left(i\right)}{P_t}\right)^{1-\varepsilon}\dd i,\\
P_t^{1-\varepsilon}&=\int_0^1P_t\left(i\right)^{1-\varepsilon}\dd i,
\qquad
P_t=\left(\int_0^1P_t\left(i\right)^{1-\varepsilon}\dd i\right)^{\frac{1}{1-\varepsilon}}.
\end{tightalign}
\blue{Why $\varepsilon$ is the elasticity of substitution across varieties?}
\[
C_t\left(i\right)=\left(\frac{P_t\left(i\right)}{P_t}\right)^{-\varepsilon}C_t,
\qquad
C_t\left(j\right)=\left(\frac{P_t\left(j\right)}{P_t}\right)^{-\varepsilon}C_t
\Rightarrow
\frac{C_t\left(i\right)}{C_t\left(j\right)}=\left(\frac{P_t\left(i\right)}{P_t\left(j\right)}\right)^{-\varepsilon}.
\]
\[
\log\frac{C_t\left(i\right)}{C_t\left(j\right)}=-\varepsilon\log\frac{P_t\left(i\right)}{P_t\left(j\right)},
\qquad
\frac{\dd\log\left\{C_t\left(i\right)/C_t\left(j\right)\right\}}{\dd\log\left\{P_t\left(i\right)/P_t\left(j\right)\right\}}=-\varepsilon.
\]
\[
\sigma_{ij}=-\frac{\dd\log\left\{C_t\left(i\right)/C_t\left(j\right)\right\}}{\dd\log\left\{P_t\left(i\right)/P_t\left(j\right)\right\}}=\varepsilon,
...
\[
\mathcal L=\E_t\sum_{t=0}^{\infty}\beta^t\left\{U\left(C_t,N_t\right)+\lambda_t\left(B_{t-1}+W_tN_t+T_t-P_tC_t-Q_tB_t\right)\right\}.
\]
\begin{tightalign}
...
\end{tightalign}
\[\left(1\right)\&\left(2\right)\Rightarrow -\frac{U_{N,t}}{U_{C,t}}=\frac{W_t}{P_t}\quad\text{Labor--leisure condition (intratemporal).}
\]
\[\left(1\right)\&\left(3\right)\Rightarrow Q_t=\beta\E_t\left\{\frac{U_{C,t+1}}{U_{C,t}}\frac{P_t}{P_{t+1}}\right\}
...
\[
U\left(C_t,N_t\right)=\frac{C_t^{1-\sigma}}{1-\sigma}-\frac{N_t^{1+\varphi}}{1+\varphi},
...
\qquad
\hat c_t=\E_t\hat c_{t+1}-\frac{1}{\sigma}\left(\hat i_t-\hat\Pi_{t+1}\right).
\]
\[
Q_t=e^{-i_t},
\qquad
...
\]
\section{Firm's problem: two simultaneous problems (isomorphic)}
\red{1. Cost minimization for each producer in the continuum}
\[
\min_{N_t\left(i\right)} W_tN_t\left(i\right)
\quad\text{s.t.}\quad Y_t\left(i\right)=A_tN_t\left(i\right)^{1-\alpha}.
\]
\[
\mathcal L=W_tN_t\left(i\right)+\mu_t\left[Y_t\left(i\right)-A_tN_t\left(i\right)^{1-\alpha}\right].
\]
\[
\frac{\partial\mathcal L}{\partial N_t\left(i\right)}=W_t-\mu_t\left(1-\alpha\right)A_tN_t\left(i\right)^{-\alpha}=0
\Rightarrow W_t=\mu_t\left(1-\alpha\right)A_tN_t\left(i\right)^{-\alpha}.
\]
\[
...
\qquad
\mu_t=MC_t=\frac{W_t}{MPL_t\left(i\right)}.
\]

\noindent
$W_t$: 1 labor's cost; \quad
$MPL_t\left(i\right)$: 1 labor's output; \quad
$\frac{W_t}{MPL_t\left(i\right)}$: cost per unit of output.

\par\smallskip
\par\smallskip
\red{2. Profit maximization by choosing price.}
\[
\max_{P_t\left(i\right)}
\;
P_t\left(i\right)Y_t\left(i\right)-W_tN_t\left(i\right)
=
\max_{P_t\left(i\right)}
\;
P_t\left(i\right)Y_t\left(i\right)
-
W_t
\left(
\frac{Y_t\left(i\right)}{A_t}
...
\[
\frac{\partial Profit}{\partial P_t\left(i\right)}
=
Y_t\left(i\right)
+
P_t\left(i\right)\frac{\partial Y_t\left(i\right)}{\partial P_t\left(i\right)}
-
W_t
\frac{1}{1-\alpha}
\left(
...
\frac{\partial Y_t\left(i\right)}{\partial P_t\left(i\right)}
\frac{1}{A_t}
=0.
\tag{*}
\]
...
MC_t
=
\frac{\partial}{\partial Y_t\left(i\right)}
\left[
W_t
\left(
\frac{Y_t\left(i\right)}{A_t}
...
P_t\left(i\right)-MC_t
\right]
\frac{Y_t\left(i\right)}{P_t\left(i\right)}
=0.
\]

Divide both sides by \(Y_t\left(i\right)\):
\[
1
-
\varepsilon
\frac{P_t\left(i\right)-MC_t}{P_t\left(i\right)}
=0.
\]

\[
1-\varepsilon+\varepsilon\frac{MC_t}{P_t\left(i\right)}
=0.
\]
...
\par\smallskip
\blue{When $\varepsilon\to\infty\Rightarrow \mu\to1$, perfect competition, $P_t\left(i\right)=MC_t$.}

\blue{In general $\varepsilon$ is finite, $\mu>1$, $P_t\left(i\right)>MC_t\Rightarrow$ inefficiency, low employment and output.}
\par\smallskip
\par\smallskip
...
\red{Nesting the two:}
\[
C\left(Y_t\left(i\right)\right)=\min_{N_t\left(i\right)}W_tN_t\left(i\right)
\quad\text{s.t.}\quad Y_t\left(i\right)=A_tN_t\left(i\right)^{1-\alpha}.
\]
Then solve
\[
\max_{P_t\left(i\right)}\left\{P_t\left(i\right)Y_t\left(i\right)-C\left(Y_t\left(i\right)\right)\right\}
\quad\text{s.t.}\quad
Y_t\left(i\right)=\left(\frac{P_t\left(i\right)}{P_t}\right)^{-\varepsilon}C_t.
\]
When $\alpha=0$, marginal cost connects the two problems:
\[
MC_t=\frac{\partial C\left(Y_t\left(i\right)\right)}{\partial Y_t\left(i\right)}.
\]
\section{Sticky-price equilibrium}
Calvo (1983) pricing: Each period a measure $\left(1-\theta\right)$ fraction of the continuum of firms reset their prices, the other $\theta$ keeps price unchanged.
\[
...
Thus
\[
P_t=\left[\theta P_{t-1}^{1-\varepsilon}+\left(1-\theta\right)\left(P_t^*\right)^{1-\varepsilon}\right]^{\frac{1}{1-\varepsilon}}.
\]
...
\end{tightalign}
\[
\Rightarrow \hat\pi_t=\left(1-\theta\right)\left(\hat p_t^*-
\hat p_{t-1}\right).
\]
...
Choose $P_t^*$ to maximize the expected PV of profits while this price remains effective, $P_t^*=P_t^*\left(i\right)$.
\[
V_t\left(i\right)=\max_{P_t^*}\sum_{k=0}^{\infty}\theta^k\E_t\left[Q_{t,t+k}\left(P_t^*Y_{t+k|t}-\psi_{t+k}\left(Y_{t+k|t}\right)\right)\right]
...
\noindent
$\psi_{t+k}\left(\cdot\right)$ is the cost function.
...
\]
\[
Y_{t+k|t}=\left(\frac{P_t^*}{P_{t+k}}\right)^{-\varepsilon}C_{t+k}=\left(P_t^*\right)^{-\varepsilon}P_{t+k}^{\varepsilon}Y_{t+k},\quad \left(Y_t=C_t,\forall t\right).
\]
\[\left(*\right)\Rightarrow
P_t^*=\mu\frac{\E_t\sum_{k=0}^{\infty}\left(\beta\theta\right)^k\widetilde Q_{t,t+k}P_{t+k}^{\varepsilon}Y_{t+k}\varphi_{t+k|t}}{\E_t\sum_{k=0}^{\infty}\left(\beta\theta\right)^k\widetilde Q_{t,t+k}P_{t+k}^{\varepsilon}Y_{t+k}}.
\]
\[
P_t^*\E_t\sum_{k=0}^{\infty}\left(\beta\theta\right)^k\widetilde Q_{t,t+k}P_{t+k}^{\varepsilon}Y_{t+k}
=\mu\E_t\sum_{k=0}^{\infty}\left(\beta\theta\right)^k\widetilde Q_{t,t+k}P_{t+k}^{\varepsilon}Y_{t+k}\varphi_{t+k|t}.
\]
Let
\[
A_t\equiv\sum_{k=0}^{\infty}\left(\beta\theta\right)^k\widetilde Q_{t,t+k}P_{t+k}^{\varepsilon}Y_{t+k},
\qquad
a_{t+k}\equiv\left(\beta\theta\right)^k\widetilde Q_{t,t+k}P_{t+k}^{\varepsilon}Y_{t+k}.
\]
\[
...
\par\smallskip
\[
\widehat{LHS}=\hat p_t^*+\left(1-\beta\theta\right)\E_t\sum_{k=0}^{\infty}\left(\beta\theta\right)^k\left(\hat{\widetilde q}_{t,t+k}+\varepsilon\hat p_{t+k}+\hat y_{t+k}\right).
\]
\[
B_t\equiv\E_t\sum_{k=0}^{\infty}\left(\beta\theta\right)^k\widetilde Q_{t,t+k}P_{t+k}^{\varepsilon}Y_{t+k}\varphi_{t+k|t},
\qquad
...
\end{tightalign}
\[
\widehat{RHS}=\left(1-\beta\theta\right)\E_t\sum_{k=0}^{\infty}\left(\beta\theta\right)^k\left(\hat{\widetilde q}_{t,t+k}+\varepsilon\hat p_{t+k}+\hat y_{t+k}+\hat\varphi_{t+k|t}\right).
\]
...
\par\smallskip
\blue{1. Weights $\sum_{k=0}^{\infty}\left(1-\beta\theta\right)\left(\beta\theta\right)^k=1$: weighted average over expected future nominal marginal cost.}
...
\hat p_t^*
=
\left(1-\beta\theta\right)
\E_t
\sum_{k=0}^{\infty}
\left(\beta\theta\right)^k
\hat p_{t+k}.
\]
...
\[
Y_t\left(i\right)=C_t\left(i\right),\quad \forall i\in\left[0,1\right].
\]
Define
\[
Y_t=\left(\int_0^1Y_t\left(i\right)^{\frac{\varepsilon-1}{\varepsilon}}\dd i\right)^{\frac{\varepsilon}{\varepsilon-1}}.
\]
Get $Y_t=C_t$. Log-linearize: $\hat y_t=\hat c_t$. Since
\[
\hat c_t=\E_t\hat c_{t+1}-\frac{1}{\sigma}\left(\hat i_t-\E_t\hat\pi_{t+1}\right),
\]
\[
\Rightarrow \hat y_t=\E_t\hat y_{t+1}-\frac{1}{\sigma}\left(\hat i_t-\E_t\hat\pi_{t+1}\right).
\]
...
\left[
\int_0^1
\left(\frac{P_t\left(i\right)}{P_t}\right)^{-\frac{\varepsilon}{1-\alpha}}
\dd i
\right]^{1-\alpha}
=
\exp\left(d_t\right).
...
\frac{P_t\left(i\right)}{P_t}=1,\ \forall i
\Rightarrow
D_t=1
\Rightarrow
d_t=0.
...
\[
Y_t\left(i\right)=\left(\frac{P_t\left(i\right)}{P_t}\right)^{-\varepsilon}Y_t.
\]

\noindent
\blue{Firms with low relative prices sell too much; firms with high relative prices sell too little.}
\[
d_t=\left(1-\alpha\right)\log\left[\int_0^1\left(\frac{P_t\left(i\right)}{P_t}\right)^{-\frac{\varepsilon}{1-\alpha}}\dd i\right].
\]
Define $p_t\left(i\right)=\log P_t\left(i\right)$, $p_t=\log P_t$, $P_t\left(i\right)/P_t=e^{p_t\left(i\right)-p_t}$. Let $\chi_i=p_t\left(i\right)-p_t$.
\[
\Rightarrow d_t=\left(1-\alpha\right)\log\left[\int_0^1\exp\left(-\frac{\varepsilon}{1-\alpha}\chi_i\right)\dd i\right].
\]

\par\smallskip
Second-order Taylor expansion around zero-inflation / zero-dispersion steady state.\\
\red{Why zero inflation $\bar\Pi=1\Leftrightarrow$ zero dispersion $P_t\left(i\right)/P_t=1\Leftrightarrow \bar d=0$?}
\[
P_t=\left[\theta P_{t-1}^{1-\varepsilon}+\left(1-\theta\right)\left(P_t^*\right)^{1-\varepsilon}\right]^{\frac{1}{1-\varepsilon}},
...
e^z
=
e^0+\frac{\left.\left(e^z\right)\right|_{z=0}}{1!}\left(z-0\right)
+\frac{\left.\left(e^z\right)\right|_{z=0}}{2!}\left(z-0\right)^2
+o\left(z^2\right).
\]
...
\int_0^1\chi_i^2\dd i
\approx
\operatorname{Var}_i\left\{p_t\left(i\right)-p_t\right\}
\quad
...
\]
\[
\hat y_t=\hat a_t+\left(1-\alpha\right)\hat n_t-\hat d_t
\Rightarrow
\hat y_t=\hat a_t+\left(1-\alpha\right)\hat n_t,
\qquad
\hat n_t=\frac{1}{1-\alpha}\left(\hat y_t-\hat a_t\right).
\]
\newpage
\section{Average real marginal cost}
\red{Average firm:}
\[
MC_t^r=\frac{W_t}{P_t}\frac{1}{MPN_t}=\frac{W_t}{P_t}\frac{1}{\left(1-\alpha\right)A_tN_t^{-\alpha}}.
\]
\begin{tightalign}
\widehat{mc}_t^r
...
\Rightarrow \hat y_{t+k}-\hat y_{t+k|t}=\varepsilon\left(\hat p_t^*-
\hat p_{t+k}\right).
\]
\[
\Rightarrow\widehat{mc}_{t+k|t}^r=\widehat{mc}_{t+k}^r-\frac{\alpha\varepsilon}{1-\alpha}\left(\hat p_t^*-
\hat p_{t+k}\right).
\]
\par\smallskip
...
\left(\hat p_t^*-\hat p_{t+k}\right),
\]
we have
\[
\hat p_t^*
=
\left(1-\beta\theta\right)E_t
\sum_{k=0}^{\infty}
\left(\beta\theta\right)^k
\left[
...
Let $k'=k+1$, $\forall k$:
\[
\beta\theta\hat p_{t+1}^*=\left(1-\beta\theta\right)\E_t\left\{\sum_{k'=1}^{\infty}\left(\beta\theta\right)^{k'}\left[\Theta\widehat{mc}_{t+k'}^r+\hat p_{t+k'}\right]\right\}.
\]
\blue{$k'$ is any index; we can use $k$.}\
\blue{(*) just take out the term of $k=0$.}
Combine (*) with log-linearized price index:
\[
\hat p_t=\theta\hat p_{t-1}+\left(1-\theta\right)\hat p_t^*.
\]
\begin{tightalign}
...
\[
P_t\left(i\right)=\frac{\varepsilon}{\varepsilon-1}\frac{W_t}{A_tF_N,t\left(N_t\left(i\right)\right)}=\mu MC_t=P_t,
\]
\[
...
NKPC:
\[
\hat\pi_t=\beta\E_t\hat\pi_{t+1}+\lambda\widehat{mc}_t^r\quad\left(\hat\pi_{t+1}=\pi_{t+1}\right)
\]
\[
\Rightarrow \hat\pi_t=\beta\E_t\hat\pi_{t+1}+\kappa\tilde y_t,
\quad \kappa=\lambda\left(\sigma+\frac{\varphi+\alpha}{1-\alpha}\right),
\quad \lambda=\frac{\left(1-\theta\right)\left(1-\beta\theta\right)}{\theta}\Theta,
...
\[
\hat r_t^n=\sigma\E_t\left\{\hat y_{t+1}^n-\hat y_t^n\right\}
=\frac{\sigma\left(1+\varphi\right)}{\varphi+\alpha+\sigma\left(1-\alpha\right)}\E_t\left\{\Delta\hat a_{t+1}\right\}
=\sigma\psi_{ya}^n\E_t\left\{\Delta\hat a_{t+1}\right\}.
\]
\[
\Rightarrow \tilde y_t=-\frac{1}{\sigma}\left(\hat i_t-\E_t\left\{\hat\pi_{t+1}\right\}-\hat r_t^n\right)+\E_t\left\{\tilde y_{t+1}\right\}.
\]
Define
\[
\hat r_t=\hat i_t-\E_t\left\{\hat\pi_{t+1}\right\}.
\]
\[
\Rightarrow \tilde y_t=-\frac{1}{\sigma}\left(\hat r_t-\hat r_t^n\right)+\E_t\left\{\tilde y_{t+1}\right\}.
\]
Assume $\lim_{T\to\infty}\E_t\left\{\tilde y_{t+T}\right\}=0$ and iterate forward:
\[
\Rightarrow \tilde y_t=-\frac{1}{\sigma}\E_t\left\{\sum_{k=0}^{\infty}\left(\hat r_{t+k}-\hat r_{t+k}^n\right)\right\}.
\]
...
NKPC:
\[
\hat\pi_t=\beta\E_t\left\{\hat\pi_{t+1}\right\}+\kappa\tilde y_t\quad\text{iterate forward}
\]
\[
\Rightarrow\hat\pi_t=\kappa\sum_{k=0}^{\infty}\beta^k\E_t\left\{\tilde y_{t+k}\right\},
...
\left\{
\begin{array}{ll}
\hat\pi_t=\kappa\sum_{k=0}^{\infty}\beta^k\E_t\left\{\tilde y_{t+k}\right\}, & \text{NKPC: determines inflation given output gap path},\\[3pt]
\tilde y_t=-\frac{1}{\sigma}\E_t\left\{\sum_{k=0}^{\infty}\left(\hat r_{t+k}-\hat r_{t+k}^n\right)\right\}, & \text{DIS: determines output gap given real interest rate gap path.}
\end{array}
\right.
\]
...
-\frac{1}{\sigma}
\left(
\hat i_t-\mathbb E_t\left\{\hat\pi_{t+1}\right\}-\hat r_t^n
\right)
+\mathbb E_t\left\{\tilde y_{t+1}\right\}
\qquad \dashrightarrow \quad \text{DIS (Demand)}
\\[0.6em]
\hat\pi_t
&=
\beta \mathbb E_t\left\{\hat\pi_{t+1}\right\}
+\kappa \tilde y_t
...
DIS:
\[
\tilde y_t=-\frac{1}{\sigma}\left[\phi_\pi\hat\pi_t+\phi_y\tilde y_t-
\E_t\left\{\hat\pi_{t+1}\right\}-\left(\hat r_t^n-v_t\right)\right]+\E_t\left\{\tilde y_{t+1}\right\}\quad \left(***\right)
\]
\[
\Rightarrow \left(1+\frac{\phi_y}{\sigma}\right)\tilde y_t+\frac{\phi_\pi}{\sigma}\hat\pi_t
=\E_t\left\{\tilde y_{t+1}\right\}+\frac{1}{\sigma}\E_t\left\{\hat\pi_{t+1}\right\}+\frac{1}{\sigma}\left(\hat r_t^n-v_t\right).
\]
\[
-\kappa\tilde y_t+\hat\pi_t=\beta\E_t\left\{\hat\pi_{t+1}\right\}.
\]
\par\smallskip
Write in matrix form in system of $\left[\tilde y_t,\hat\pi_t\right]'$:
\[
...
\begin{bmatrix}\E_t\left\{\tilde y_{t+1}\right\}\\\E_t\left\{\hat\pi_{t+1}\right\}\end{bmatrix}
+\begin{bmatrix}1/\sigma\\0\end{bmatrix}\left(\hat r_t^n-v_t\right).
\]
\[
\begin{bmatrix}\tilde y_t\\\hat\pi_t\end{bmatrix}
=M^{-1}\begin{bmatrix}1&1/\sigma\\0&\beta\end{bmatrix}
\begin{bmatrix}\E_t\left\{\tilde y_{t+1}\right\}\\\E_t\left\{\hat\pi_{t+1}\right\}\end{bmatrix}
+M^{-1}\begin{bmatrix}1/\sigma\\0\end{bmatrix}\left(\hat r_t^n-v_t\right).
\]
\[
M^{-1}=\frac{1}{\left|M\right|}\begin{bmatrix}1&-\phi_\pi/\sigma\\ \kappa&1+\phi_y/\sigma\end{bmatrix}
...
\begin{bmatrix}1&1/\sigma\\0&\beta\end{bmatrix}
=\begin{bmatrix}\sigma&1-\beta\phi_\pi\\ \kappa\sigma&\kappa+\beta\left(\sigma+\phi_y\right)\end{bmatrix}\Omega,
\]
\[
...
\Rightarrow
\begin{bmatrix}\tilde y_t\\\hat\pi_t\end{bmatrix}
=A_T\begin{bmatrix}\E_t\left\{\tilde y_{t+1}\right\}\\\E_t\left\{\hat\pi_{t+1}\right\}\end{bmatrix}
...
\left\{
\begin{array}{l}
1-\det\left(A_T\right)>0\quad \left(D<1\right),\\
1-\operatorname{tr}\left(A_T\right)+\det\left(A_T\right)>0\quad \left(p\left(1\right)>0\right),\\
1+\operatorname{tr}\left(A_T\right)+\det\left(A_T\right)>0\quad \left(p\left(-1\right)>0\right).
\end{array}
\right.
\]
...
1^\circ\quad
1-
\det\left(A_T\right)=1-\frac{\beta\sigma}{\sigma+\phi_y+\kappa\phi_\pi}
=\frac{\left(1-\beta\right)\sigma+\phi_y+\kappa\phi_\pi}{\sigma+\phi_y+\kappa\phi_\pi}>0\quad \blue{\text{satisfy}},
\]
...
\end{tightalign}
\[
\Rightarrow \kappa\left(\phi_\pi-1\right)+\left(1-\beta\right)\phi_y>0\quad \blue{\text{satisfy if}}.
\]
\[
3^\circ\quad
1+\operatorname{tr}\left(A_T\right)+\det\left(A_T\right)
=\frac{\left(\sigma+\phi_y+\kappa\phi_\pi\right)+\left(\sigma+\kappa+\beta\left(\sigma+\phi_y\right)\right)+\beta\sigma}{\sigma+\phi_y+\kappa\phi_\pi}>0
\quad \blue{\text{satisfy}}.
\]
\[
\red{\text{The system is stable iff: }}\quad \kappa\left(\phi_\pi-1\right)+\left(1-\beta\right)\phi_y>0.
\]
\noindent
\par\smallskip
...
Plug (2) into (1):
\[
\psi_{\pi v}=-\frac{\kappa}{\left(1-\beta\rho_v\right)\left[\sigma\left(1-\rho_v\right)+\phi_y\right]+\kappa\left(\phi_\pi-\rho_v\right)}.
\]
\[
\hat\pi_t
=
...
\]
\[
\Lambda_v=\frac{1}{\left(1-\beta\rho_v\right)\left[\sigma\left(1-\rho_v\right)+\phi_y\right]+\kappa\left(\phi_\pi-\rho_v\right)}>0.
\]
where
\[
\kappa
...
\frac{\left(1-\theta\right)\left(1-\beta\theta\right)}{\theta}
...
\tilde y_t
=
\underbrace{-\left(1-\beta\rho_v\right)\Lambda_v}_{<0}
v_t
\]
\begin{tightalign}
\hat r_t
...
\hat i_t
=
\underbrace{\left[\sigma\left(1-\rho_v\right)\left(1-\beta\rho_v\right)-\kappa\rho_v\right]\Lambda_v}_{\text{coefficient on }v_t}
v_t.
\]
\[
...
\tilde y_t &= -\left(1-\beta\rho_v\right)\Lambda_v v_t,\\
\hat r_t &= \sigma\left(1-\rho_v\right)\left(1-\beta\rho_v\right)\Lambda_v v_t,\\
\hat i_t &= \left[\sigma\left(1-\rho_v\right)\left(1-\beta\rho_v\right)-\kappa\rho_v\right]\Lambda_v v_t.
\end{aligned}
\right.
\]
...
\qquad \rho_a\in\left[0,1\right].
\]
\[
\hat r_t^n=\sigma\E_t\left\{\hat y_{t+1}^n-\hat y_t^n\right\}=\sigma\psi_{ya}^n\E_t\left\{\Delta\hat a_{t+1}\right\}
...
where
\[
\Lambda_a=\frac{1}{\left(1-\beta\rho_a\right)\left[\sigma\left(1-\rho_a\right)+\phi_y\right]+\kappa\left(\phi_\pi-\rho_a\right)}>0.
\]
...
\psi_{ya}^n\hat a_t
-
\sigma\psi_{ya}^n\left(1-\rho_a\right)\left(1-\beta\rho_a\right)\Lambda_a\hat a_t
\\
&=
\psi_{ya}^n
\left[
1-
\underbrace{\sigma\left(1-\rho_a\right)\left(1-\beta\rho_a\right)\Lambda_a}_{\left(*\right)}
\right]\hat a_t.
\end{aligned}
\]
...
&=
1-
\sigma\left(1-\rho_a\right)\left(1-\beta\rho_a\right)
\frac{1}{
\sigma\left(1-\rho_a\right)\left(1-\beta\rho_a\right)
+
\phi_y\left(1-\beta\rho_a\right)
+
\kappa\left(\phi_\pi-\rho_a\right)
}
\\
&=
\frac{
...
\[
\begin{aligned}
\phi_y\left(1-\beta\rho_a\right)+\kappa\left(\phi_\pi-\rho_a\right)
&=
\kappa\left(\phi_\pi-1\right)
+
\left(1-\beta\right)\phi_y
+
\kappa
-
\rho_a\kappa
...
\\
&=
\underbrace{\kappa\left(\phi_\pi-1\right)+\left(1-\beta\right)\phi_y}_{\text{stable condition}}
+
\underbrace{\left(1-\rho_a\right)\kappa}_{0\le \rho_a<1}
+
\underbrace{\left(1-\rho_a\right)\beta\phi_y}_{0\le \rho_a<1}
>0.
\end{aligned}
\]
...
\[
\left(1-\alpha\right)\hat n_t=\hat y_t-
\hat a_t
=\left[\left(\psi_{ya}^n-1\right)-\sigma\psi_{ya}^n\left(1-\rho_a\right)\left(1-\beta\rho_a\right)\Lambda_a\right]\hat a_t.
\]
In calibration: $\sigma=1\Rightarrow \psi_{ya}^n=\frac{1+\varphi}{\varphi+\alpha+\sigma\left(1-\alpha\right)}=1\Rightarrow n_t$ declines when $a_t$ increases.


\par\smallskip
\section{\texorpdfstring{Equilibrium with exogenous money supply: $\Delta \hat m_t$}{Equilibrium with exogenous money supply: Delta mhat t}}
\[
\frac{M_t}{P_t}=L\left(i_t,Y_t\right).
\]
...
DIS
\Rightarrow
\left(1+\sigma\eta\right)\tilde y_t
=
\sigma\eta\E_t\left\{\tilde y_{t+1}\right\}
+
\widehat{\mathbb{M}}_t
+
\eta\E_t\left\{\hat\pi_{t+1}\right\}
+
\eta\hat r_t^n
-
\hat y_t^n.
...
\qquad \hat i_t\uparrow:\text{ no liquidity effect }\left(\sigma=1\right),
...
\red{DIS:}
\[
\tilde y_t=-\frac{1}{\sigma}\left(\hat i_t-
\E_t\hat\pi_{t+1}\right)+
\E_t\tilde y_{t+1}.
\]
Set $\hat y_t^n=0$:
\[
\hat i_t=\frac{1}{\eta}\left(\tilde y_t-
\widehat{\mathbb{M}}_t\right).
\]
...
\]
\[
\hat i_t<0\Leftrightarrow \sigma\left(\tilde y_t-\E_t\tilde y_{t+1}\right)>\E_t\hat\pi_{t+1}.
\]
